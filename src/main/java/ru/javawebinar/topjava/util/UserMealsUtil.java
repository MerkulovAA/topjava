...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        System.out.println(getFilteredWithExceededByStream(mealList, LocalTime.of(7, 0), LocalTime.of(12, 0), 1500));
    }
    ...
    private static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime,
        ...
        for (UserMeal meals : mealList) {
            if (dateToCalories.isEmpty()) {
                dateToCalories.put(meals.getLocalDate(), meals.getCalories());
            } else {
                dateToCalories.merge(meals.getLocalDate(), meals.getCalories(), (oldVal, newVal) -> oldVal + newVal);
            }
        }
    ...
    private static List<UserMealWithExceed> getFilteredWithExceededByStream(List<UserMeal> mealList, LocalTime startTime,
        ...
        return mealList.stream()
                ...
                .map(s -> new UserMealWithExceed(s.getDateTime(), s.getDescription(), s.getCalories(),
                        map.get(s.getLocalDate()) > caloriesPerDay))
